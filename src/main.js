...
app.use(VueNativeSock, "ws://localhost:8090/emos/socket", {
    "format": "json"
});
...
//配置JS生成PDF的公共函数
import html2Canvas from 'html2canvas'   // 将页面渲染为图片
import JsPDF from 'jspdf'   // 将图片列表导出为pdf
...
//后端项目的URL根路径
let baseUrl = "http://localhost:8090/emos/"
...
app.config.globalProperties.$get = function (url, data, async, fun) {
    $.ajax({
        url: baseUrl + url,
        type: "GET",
        xhrFields: {
