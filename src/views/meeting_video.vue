...
export default {
				...
				if (!checkResult.result) {
					...
					if (that.meetingStatus) {
						...
						client.on('stream-added', event => {
							...
							that.$get('user/searchNameAndDept', { "userId": userId }, true, function(resp) {
								let name = resp.name;
								let dept = resp.dept;
						...
						client
							...
							.then(() => {
								...
								//把自己添加到上线用户列表中
								that.$get('user/searchNameAndDept', { "userId": that.userId }, true, function(resp) {
									let name = resp.name;
									let dept = resp.dept;
	...
	created: function() {
		...
		that.$get('meeting/searchOnlineMeetingMembers', {"meetingId":that.meetingId}, true, function(resp) {
			let list = resp.list;
		...
		that.$get('meeting/searchRoomIdByUUID', {"uuid":that.uuid}, true, function(resp) {
			if (resp.roomId == null) {
				that.$message({
					message: '不存在视频会议室',
					type: 'error',
