            ...
            <el-table-column header-align="center" align="center" width="150" label="操作">
                <template #default="scope">
                    ...
                    <el-button
                            type="text"
                            size="medium"
                            :disabled="!((scope.row.status === '待审批' || scope.row.status === '未开始') && scope.row.isCreator)"
            ...
            that.$http('meeting/searchOnlineMeetingByPage', 'POST', data, true, function (resp) {
                ...
                for (let one of page.list) {
                    if (one.status === 1) {
                        ...
                        one.status = '已结束';
                    } else if (one.status === 6) {
                        one.status = '未审批';
                    }
                    let minute = dayjs(new Date()).diff(dayjs(`${one.date} ${one.start}`), "minute")
                    one.canEnterMeeting = one.mine && ((minute >= -10 && minute <= 0 && one.status === "未开始") || one.status === "进行中");
                }
