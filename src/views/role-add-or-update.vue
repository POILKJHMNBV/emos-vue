    ...
    >
        <el-form :model="dataForm" ref="dataForm" :rules="dataRule" label-width="60px">
            <el-form-item label="角色" prop="roleName">
                <el-input v-model="dataForm.roleName" size="medium" style="width:40%" :readonly="systemic" clearable/>
                <span class="note">（ 角色名称必须是2~10个字符之间，且不能重复 ）</span>
            </el-form-item>
            <el-form-item label="备注" prop="desc">
                <el-input v-model="dataForm.desc" style="width:40%" size="medium" maxlength="20" clearable/>
    ...
    methods: {
        init: function (id, systemic) {
            ...
            that.$nextTick(() => {
                ...
                if (id) {
                    that.$get('role/searchById', {roleId: id}, true, function (resp) {
        ...
        dataFormSubmit: function () {
            ...
            this.$refs['dataForm'].validate(valid => {
                if (valid) {
                    ...
                    //判断用户选择的权限和原来的权限是否一致？
                    that.dataForm.changed = that.dataForm.permissions.join() !== that.oldPermissions.join();
